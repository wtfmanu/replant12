...
try {
  ...
  const mobile = await evaluate(client, `(() => ({
    ...
    duplicateHeader: getComputedStyle(document.querySelector('.mobile-header')).display,
    decorativeLeaf: getComputedStyle(document.querySelector('.range-leaf')).display,
    overflow: document.documentElement.scrollWidth > window.innerWidth + 1
  }))()`);
  if (
    mobile.width !== 390 ||
    !mobile.linkFirst ||
    mobile.bottomNav === "none" ||
    mobile.duplicateHeader !== "none" ||
    mobile.decorativeLeaf !== "none" ||
    mobile.overflow
  ...
  console.log("- Mobiler Doppel-Header entfernt und runder grüner Slider aktiviert");
