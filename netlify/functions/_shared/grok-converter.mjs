...
import { normalizeConvertedRecipe } from "./fallback-converter.mjs";
...
export async function convertRecipeWithGrok(input, options = {}) {
  const apiKey = String(options.apiKey || process.env.XAI_API_KEY || "").trim();
  if (!apiKey) throw new AppError("Grok ist noch nicht eingerichtet.", 503, "AI_NOT_CONFIGURED");
...
JSON-Struktur:
{
  ...
  try {
    ...
    if (error?.name === "AbortError") throw new AppError("Grok hat zu lange für eine Antwort gebraucht.", 504, "AI_TIMEOUT");
    throw new AppError("Grok konnte nicht erreicht werden.", 502, "AI_NETWORK_ERROR");
  } finally {
  ...
  if (!response.ok) {
    let message = "Grok hat die Anfrage abgelehnt.";
    try {
      const parsed = JSON.parse(raw);
      const apiMessage = cleanString(parsed?.error?.message, 300);
      if (response.status === 401 || response.status === 403) message = "Der Grok API-Key wurde nicht akzeptiert.";
      else if (response.status === 429) message = "Das Grok-Limit wurde erreicht. Bitte später erneut versuchen.";
  ...
  try {
    ...
    throw new AppError("Grok hat kein gültiges Rezeptformat geliefert.", 502, "AI_RESPONSE_INVALID");
  }
  const recipe = normalizeConvertedRecipe(payload, input.sourceUrl);
  if (!recipe.title || recipe.ingredients.length < 2 || recipe.steps.length < 1) {
    throw new AppError("Die Grok-Antwort war unvollständig.", 502, "AI_RESPONSE_INCOMPLETE");
  }
  return { recipe, model };
}

export const GROK_DEFAULT_MODEL = DEFAULT_MODEL;
